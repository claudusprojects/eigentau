...
const db = new Database(join(dbDir, 'eigentau.db'));
...
export function saveRoute(data) {
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)`).run(data.query, JSON.stringify(data.decomposition), JSON.stringify(data.results), data.synthesis, data.quality, data.totalLatencyMs, JSON.stringify(data.facultiesUsed), JSON.stringify(data.subnetsUsed));
}
...
export function getRouteCount() {
    return db.prepare('SELECT COUNT(*) as n FROM routes').get().n;
}
export function getWeights() {
    return db.prepare('SELECT * FROM weights ORDER BY weight DESC').all();
}
export function updateWeight(faculty, quality) {
    const EMA = 0.8;
    const row = db.prepare('SELECT * FROM weights WHERE faculty = ?').get(faculty);
    if (!row)
        return;
...
export function addInsight(cycle, type, text) {
...
export function addAgentFile(agentId, filename, content, mimeType = 'text/plain') {
...
export function getAgentFileContent(agentId, fileId) {
    return db.prepare('SELECT * FROM agent_files WHERE agent_id = ? AND id = ?').get(agentId, fileId);
}
export function deleteAgentFile(agentId, fileId) {
    db.prepare('DELETE FROM agent_files WHERE agent_id = ? AND id = ?').run(agentId, fileId);
}
export function getAgentKnowledge(agentId) {
    const files = db.prepare('SELECT filename, content FROM agent_files WHERE agent_id = ?').all(agentId);
    if (files.length === 0)
        return '';
