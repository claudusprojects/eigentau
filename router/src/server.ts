...
import { saveRoute, getRecentRoutes, getRouteCount, getWeights, updateWeight, getInsights, getStats, createAgent, getAgent, listAgents, deleteAgent, getMessages, getAgentTasks, getAgentCount, createApiKey, validateApiKey, listApiKeys, deleteApiKey, addAgentFile, getAgentFiles, deleteAgentFile, getAgentFileContent } from './db.js';
...
app.get('/api/strategy', (_req, res) => {
  ...
  const weights = stats.weights as any[];

  // Compute regime from weight distribution
  const maxW = Math.max(...weights.map((w: any) => w.weight));
  const minW = Math.min(...weights.map((w: any) => w.weight));
  ...
  const avgLatency = stats.totalRoutes > 0
    ? Math.round((getRecentRoutes(20) as any[]).reduce((s: number, r: any) => s + r.total_latency_ms, 0) / Math.min(stats.totalRoutes, 20))
    : 0;
  const activeFaculties = weights.filter((w: any) => w.total_uses > 0).length;
...
function authMiddleware(req: Request, res: Response, next: NextFunction) {
  const key = req.headers.authorization?.replace('Bearer ', '') || req.query.api_key as string;
...
v1.post('/route', async (req, res) => {
  ...
  try {
    ...
    const synthesis = await synthesize(query, results);
    const totalLatencyMs = Date.now() - start;
    for (const f of synthesis.faculties_used) updateWeight(f, synthesis.quality);
    saveRoute({
      query, decomposition,
